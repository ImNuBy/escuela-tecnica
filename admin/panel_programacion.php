...
// Obtener información de la orientación Programación
$stmt = $pdo->prepare("SELECT id FROM orientaciones WHERE nombre LIKE '%programación%' OR nombre LIKE '%programacion%'");
$stmt->execute();
$orientacion_prog = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$orientacion_prog) {
    // Si no existe, usar la segunda orientación como fallback
    $stmt = $pdo->query("SELECT id FROM orientaciones ORDER BY id LIMIT 1,1");
    $orientacion_prog = $stmt->fetch(PDO::FETCH_ASSOC);
}

$orientacion_id = $orientacion_prog['id'];

// Obtener años de Programación (4to, 5to, 6to, 7mo) - CORREGIDO
$stmt = $pdo->prepare("
...
$años_prog = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Estadísticas generales de Programación
$stmt = $pdo->prepare("
...
// Obtener horarios de Programación
$stmt = $pdo->prepare("
...
");
$stmt->execute([$orientacion_id]);
$horarios_prog = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Organizar horarios por día y año
$horarios_organizados = [];
foreach ($horarios_prog as $horario) {
    $año = $horario['año'];
...
// Noticias dirigidas a Programación
$stmt = $pdo->prepare("
    SELECT n.*, u.nombre, u.apellido
    FROM noticias n
    JOIN usuarios u ON n.autor_id = u.id
    WHERE (n.dirigido_a = 'todos' OR n.dirigido_a = 'programacion') AND n.activo = 1
    ORDER BY n.fecha_publicacion DESC
    LIMIT 5
");
$stmt->execute();
$noticias_prog = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// CORREGIDO: Información actualizada para incluir 7° año
$años_programacion = [
    4 => ['nombre' => '4° Año', 'descripcion' => 'Introducción a la Programación'],
    5 => ['nombre' => '5° Año', 'descripcion' => 'Programación Intermedia'],
    6 => ['nombre' => '6° Año', 'descripcion' => 'Programación Avanzada'],
...
<head>
    ...
    <title>Panel Programación - Sistema Escolar</title>
    ...
    <div class="main-container">
        ...
        <main class="content">
            <div class="page-header">
                <h1>💻 Panel Programación</h1>
                <p>Gestión integral de la orientación Programación - 4°, 5°, 6° y 7° Año</p>
            </div>
            ...
            <div class="programacion-container">
                ...
                <div class="estadisticas-prog">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card stat-años">
                            <div class="stat-icon">🎯</div>
                            <div class="stat-info">
                                <h3><?php echo count($años_prog); ?></h3>
                ...
                <div class="info-carrera">
                    <h2>🚀 Carrera de Programación</h2>
                    <div class="carrera-grid">
                        <?php foreach ($años_programacion as $num_año => $info): ?>
                ...
                <!-- Tabs -->
                <div class="tabs-prog">
                    <button class="tab-button active" onclick="cambiarTab('materias-horarios')">🖥️ Materias y Horarios</button>
                ...
                <div id="materias-horarios" class="tab-content active">
                    <h2>🖥️ Materias por Año</h2>
                    
                    <?php if (!empty($años_prog)): ?>
                        <?php foreach ($años_prog as $año): ?>
                                    ...
                                    <div class="materias-grid">
                                        <?php foreach ($materias_por_año[$año['año']] as $materia): ?>
                                            <div class="materia-card">
                                                ...
                                                <?php
                                                $horarios_materia = array_filter($horarios_prog, function($h) use ($materia) {
                    ...
                    <?php else: ?>
                        <div class="sin-años">
                            <h3>⚠️ No se encontraron años de Programación</h3>
                ...
                <div id="estudiantes" class="tab-content">
                    <h2>👨‍💻 Estudiantes de Programación</h2>
                ...
                <div id="horarios-completos" class="tab-content">
                    <h2>📅 Horarios Semanales de Programación</h2>
                    ...
                    <div class="tecnologias-grid">
                        <div class="tech-category">
                            ...
                            <div class="tech-items">
                                ...
                                <div class="tech-item">JavaScript Básico</div>
                                <div class="tech-item">Algoritmos</div>
                                <div class="tech-item">Python</div>
                            </div>
                        </div>
                        
                        <div class="tech-category">
                            <h3>5° Año - Desarrollo</h3>
                            <div class="tech-items">
                        ...
                        <div class="tech-category">
                            <h3>6° Año - Avanzado</h3>
                            <div class="tech-items">
                                <div class="tech-item">React.js</div>
                                <div class="tech-item">Node.js</div>
                                <div class="tech-item">MongoDB</div>
                                <div class="tech-item">APIs REST</div>
                            </div>
                        </div>
                        ...
                        <div class="tech-category">
                            ...
                            <div class="tech-items">
                                <div class="tech-item">Full Stack Development</div>
                                <div class="tech-item">Docker & DevOps</div>
                ...
                <div id="noticias" class="tab-content">
                    <h2>📢 Noticias de Programación</h2>
                    
                    <?php if (!empty($noticias_prog)): ?>
                        <div class="noticias-prog-grid">
                            <?php foreach ($noticias_prog as $noticia): ?>
                    ...
                    <?php else: ?>
                        <div class="sin-noticias-prog">
                            ...
                            <p>No hay noticias dirigidas a Programación en este momento.</p>
                        </div>
                    <?php endif; ?>
                </div>
                    ...
                    <div class="acciones-grid">
                        <a href="../admin/usuarios.php" class="accion-card">
                            ...
                            <div class="accion-texto">
                                ...
                                <p>Administrar estudiantes de programación</p>
                            </div>
                        </a>
                        ...
                        <a href="../admin/academico.php" class="accion-card">
                            ...
                            <div class="accion-texto">
                                ...
                                <p>Configurar materias de programación</p>
                            </div>
                        </a>
